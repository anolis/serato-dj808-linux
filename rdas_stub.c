/*
 * rdas_stub.c — Fake Roland DJ-808 ASIO driver (RDAS1174.DLL replacement)
 *
 * The real RDAS1174.DLL uses DeviceIoControl to talk to the Roland USB audio
 * kernel driver, which doesn't exist under Wine.  It therefore returns 0 Hz /
 * 0 buffer-size on every ASIO query, causing Serato DJ Pro to fail the
 * hardware connection check and disable Dual Control.
 *
 * This stub implements the full IASIO COM vtable, returns plausible values
 * (44100 Hz, 512-sample buffers, 4-in/4-out), and provides zero-filled dummy
 * audio buffers so Serato's connection check passes.  Actual audio continues
 * through the Windows Audio (winmm/WASAPI) path that Wine already services.
 *
 * Compile (64-bit Windows DLL):
 *   x86_64-w64-mingw32-gcc -shared -O2 -o RDAS1174.DLL rdas_stub.c \
 *       rdas_stub.def -Wl,--export-all-symbols -lkernel32 -luser32 -lwinmm
 */
...
static void WINAPI asio_getName(IASIO *t, char *n) { strcpy(n, "DJ-808 ASIO"); }
...
/* ── Callback / audio routing state ──────────────────────────────────────── */

static ASIOCallbacks *g_cb       = NULL;
static long           g_buf_sz   = 512;
static volatile long  g_running  = 0;
static HANDLE         g_thread   = NULL;
/* g_out_slot[asio_channel] = index into g_bufs[], or -1 if not mapped */
static int            g_out_slot[MAX_CH];

/* PCM output buffers: double-buffered stereo int16 for waveOut */
...
/*
 * Drive Serato's audio engine via the ASIO bufferSwitch callback and route
 * the output to the default Windows Audio device using waveOut.
 * Falls back to a pure-Sleep loop if waveOut cannot be opened.
 */
static unsigned __stdcall callback_thread(void *arg)
{
    ...
    if (waveOutOpen(&hwo, WAVE_MAPPER, &wfx,
        ...
        for (int i = 0; i < 2; i++) {
            ...
            /* prime with silence so the device starts immediately */
    ...
    while (g_running) {
        if (use_wo) {
            /* wait for the oldest queued buffer to finish playing */
            while (!(hdr[wave_idx].dwFlags & WHDR_DONE) && g_running)
                WaitForSingleObject(evt, 20);
            if (!g_running) break;

            /* ask Serato to fill ASIO output buffers [asio_idx] */
            if (g_cb && g_cb->bufferSwitch)
                g_cb->bufferSwitch(asio_idx, ASIOTrue);

            /* convert int32 ASIO → int16 interleaved and re-queue */
            ...
            /* fallback: correct-rate Sleep, no audio output */
            DWORD ms = (DWORD)((g_buf_sz * 1000UL) / 44100UL);
            Sleep(ms > 0 ? ms : 1);
